...
|---|---|---|---|---|
| **Perakende satış** | Superstore Sales | Tableau örnek verisi / Kaggle kopyaları | Kategori, alt kategori, bölge, tarih, satış, kâr kolonları → karşılaştırma + trend + kompozisyon soruları; belirli bir ürün kategorisi senaryosuna kolay uyarlanır | Serbest örnek veri; Kaggle sürümünün lisans sayfasını kontrol et |
...
| **Enerji tüketimi** | Individual Household Electric Power Consumption | UCI ML Repository | Zaman serisi güç tüketimi → trend + anomali soruları; cihaz enerji profili anlatısına uygun | UCI: CC BY 4.0 (doğrula) |
...
| **Müşteri analitiği** | Mall Customers | Kaggle | Yaş, gelir, harcama skoru → ilişki + dağılım + segmentasyon soruları ("yaş ↔ yıllık harcama" gibi ilişki örnekleri) | Kaggle lisans sayfasını kontrol et |
...
**Seçim önerisi:** Superstore + UCI Appliances Energy + Mall Customers. Üçü de küçük/orta boyutlu (ücretsiz altyapıya uygun), temiz ve yaygın bilinen setler; benchmark'ı savunmak kolay.

> Not: Bu ön araştırmada enerji seti için Household Power önerilmişti; final projede **Appliances Energy Prediction**'a geçildi (cihaz bazlı tüketim + ortam sensörleri ilişki/anomali sorularına daha uygun) ve saatlik özeti Space demosu için türetildi. Nihai setler ve temizlik adımları `data/README.md` ile [data.md](data.md)'de belgelidir.

**Senaryo uyarlaması:** Veri setlerine dokunulmaz (provenance korunur); tematik bağlam **soru setinde** kurulur: "Belirli bir kategoride son çeyrek satış trendi", "Enerji tüketiminin gün içi dağılımı", "Müşteri yaşı ile harcama ilişkisi" gibi. Gerekirse Superstore'un kategori adları senaryo gereği hedef kategorilerle eşlenir ve bu eşleme [data.md](data.md)'de açıkça belgelenir (overclaim yok).
