"""Rubric calibration (Task B4/T1).

Ten hand-built cases, each one a failure mode observed in this project, checked
against the rubric. The point is not code coverage: it is to show that the
rubric orders candidates the way a reviewer would, before it is used to label
450 pairs unattended.

Every case states what the reviewer's judgement is and why. If the rubric
disagrees with the reviewer, the rubric is wrong and gets fixed — the cases are
the specification.
...
REF_TREND = {"x_axis": "order_date", "y_axis": "sales", "chart_family": ["line"],
             ...
             "transform": {"groupby": "month(order_date)", "agg": "sum",
                           "filter": None, "sort": "date_asc", "limit": None}}
...
CASES = [
    ...
    ("correct granularity beats wrong granularity", "data_analyst",
     ...
     {"target_columns": ["order_date", "sales"],
      "transform": {"groupby": "day(order_date)", "agg": "sum",
                    "filter": None, "sort": "date_asc", "limit": None}},
