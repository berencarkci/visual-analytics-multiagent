...
This script fixes the mismatch by decomposing every single call example into the formats the agents actually use at inference time:
...
#
# Three variants per focus. 
# One variant would teach the model to reproduce a fixed string, which is what the deterministic template fallback already does  for free, the point of the LLM call is a sentence that reads naturally while staying pinned to the computed values.
...
def _prepare(example: dict, frames: dict) -> tuple | None:
    ...
    plan = TransformPlan(transform=rec.transform, target_columns=target_columns, result_rows=int(len(prepared)), notes=notes)
    return df, prepared, x_col, y_col, plan, target_columns
#################################


# Example builders for each format:
def build_agent_examples(examples: list[dict], frames: dict, schemas: dict) -> dict[str, list[dict]]:
    ...
    for ex in examples:
        ...
        if intent == "distribution" and t["transform"].get("groupby"):
            # categorical distribution (v4): the target is counts for every category, so the insight must describe groups, not the median of the counts.
...
def main() -> int:
    ...
    # Intent only bank (v3): supervisor format records for lure patterns with no clean executable chart target. 
    ...
    print("contamination: single call questions are checked by make_sft_data.py, intent only questions are checked above in this script")
    return 0
